...
function resolveWorkspace(workspacePath, requestedPath) {
...
function extractFilePaths(body) {
    ...
    for (const match of body.matchAll(FILE_PATH_REGEX)) {
        ...
        if (!FILE_EXTENSION_REGEX.test(cleaned))
            continue;
        // Skip things that look like URL routes
        if (URL_ROUTE_PATTERN.test(cleaned))
            continue;
        paths.add(cleaned);
    }
    return [...paths];
...
const plugin = definePlugin({
    async setup(ctx) {
        ...
        ctx.data.register("fileList", async (params) => {
            const projectId = params.projectId;
            const companyId = typeof params.companyId === "string" ? params.companyId : "";
        ...
        ctx.data.register("fileContent", async (params) => {
            const projectId = params.projectId;
            const companyId = typeof params.companyId === "string" ? params.companyId : "";
            const workspaceId = params.workspaceId;
            ...
            const workspace = workspaces.find((w) => w.id === workspaceId);
            if (!workspace)
                return { content: null, error: "Workspace not found" };
            const workspacePath = sanitizeWorkspacePath(workspace.path);
            if (!workspacePath)
                return { content: null, error: "Workspace has no path" };
        ...
        ctx.actions.register("writeFile", async (params) => {
            const projectId = params.projectId;
            const companyId = typeof params.companyId === "string" ? params.companyId : "";
            const workspaceId = params.workspaceId;
