...
import { PLUGIN_STATE_SCOPE_KINDS, definePlugin, runWorker, } from "@paperclipai/plugin-sdk";
import { DEFAULT_CONFIG, JOB_KEYS, PLUGIN_ID, RUNTIME_LAUNCHER, SAFE_COMMANDS, STREAM_CHANNELS, TOOL_NAMES, WEBHOOK_KEYS, } from "./constants.js";
const recentRecords = [];
const runtimeLaunchers = new Map();
let currentContext = null;
let lastProcessResult = null;
function isScopeKind(value) {
    return typeof value === "string" && PLUGIN_STATE_SCOPE_KINDS.includes(value);
}
...
async function writeInstanceState(ctx, stateKey, value) {
    await ctx.state.set({ scopeKind: "instance", stateKey }, value);
}
async function readInstanceState(ctx, stateKey) {
    return await ctx.state.get({ scopeKind: "instance", stateKey });
}
async function resolveWorkspace(ctx, companyId, projectId, workspaceId) {
...
function ensureInsideWorkspace(workspacePath, relativePath) {
...
async function runCuratedCommand(ctx, config, companyId, projectId, workspaceId, commandKey) {
...
function getCurrentCompanyId(params) {
...
async function registerDataHandlers(ctx) {
    ...
    ctx.data.register("overview", async (params) => {
        ...
        const entityRecords = await ctx.entities.list({ limit: 10 });
        return {
            pluginId: PLUGIN_ID,
...
async function registerActionHandlers(ctx) {
    ...
    ctx.actions.register("advance-issue-status", async (params) => {
        ...
        const issue = await ctx.issues.update(issueId, { status: status }, companyId);
        pushRecord({
            level: "info",
    ...
    ctx.actions.register("advance-goal-status", async (params) => {
        ...
        const goal = await ctx.goals.update(goalId, { status: status }, companyId);
        pushRecord({
            level: "info",
            source: "goals.update",
...
async function registerToolHandlers(ctx) {
    ctx.tools.register(TOOL_NAMES.echo, {
    ...
    ctx.tools.register(TOOL_NAMES.companySummary, {
    ...
    ctx.tools.register(TOOL_NAMES.createIssue, {
...
async function registerEventHandlers(ctx) {
    ...
    ctx.events.on(`plugin.${PLUGIN_ID}.demo-event`, async (event) => {
        pushRecord({
            level: "info",
            source: "plugin-event",
...
async function registerJobs(ctx) {
    ctx.jobs.register(JOB_KEYS.heartbeat, async (job) => {
        const payload = {
            jobKey: job.jobKey,
            runId: job.runId,
            trigger: job.trigger,
...
const plugin = definePlugin({
    ...
    async onValidateConfig(config) {
        ...
        if (Array.isArray(typed.allowedCommands)) {
            const allowed = new Set(SAFE_COMMANDS.map((command) => command.key));
