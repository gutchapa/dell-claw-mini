...
import { listServerAdapters, findServerAdapter, findActiveServerAdapter, registerServerAdapter, unregisterServerAdapter, isOverridePaused, setOverridePaused, } from "../adapters/registry.js";
import { getAdapterSessionManagement } from "@paperclipai/adapter-utils";
import { listAdapterPlugins, addAdapterPlugin, removeAdapterPlugin, getAdapterPluginByType, getAdapterPluginsDir, getDisabledAdapterTypes, setAdapterDisabled, } from "../services/adapter-plugin-store.js";
 ...
 */
function readAdapterPackageVersionFromDisk(record) {
    try {
...
function buildAdapterInfo(adapter, externalRecord, disabledSet) {
 ...
 */
async function normalizeLocalPath(rawPath) {
 ...
 */
function registerWithSessionManagement(adapter) {
    const wrapped = {
        ...adapter,
...
export function adapterRoutes() {
    ...
    router.get("/adapters", async (_req, res) => {
        ...
        const result = registeredAdapters.map((adapter) => buildAdapterInfo(adapter, externalRecords.get(adapter.type), disabledSet)).sort((a, b) => a.type.localeCompare(b.type));
        res.json(result);
    });
    /**
    ...
    router.post("/adapters/install", async (req, res) => {
        assertBoard(req);
        const { packageName, isLocalPath = false, version } = req.body;
    ...
    router.delete("/adapters/:type", async (req, res) => {
        ...
        if (externalRecord.packageName && !externalRecord.localPath) {
            try {
                ...
                logger.info({ type: adapterType, packageName: externalRecord.packageName }, "npm uninstall completed for external adapter");
            }
            catch (err) {
                logger.warn({ err, type: adapterType, packageName: externalRecord.packageName }, "npm uninstall failed for external adapter; continuing with unregister");
            }
        }
