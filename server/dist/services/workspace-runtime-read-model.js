...
function runtimeServiceIdentityKey(row) {
    if (row.reuseKey)
        return row.reuseKey;
    return [
        row.scopeType,
...
export function selectCurrentRuntimeServiceRows(rows) {
...
export async function listCurrentRuntimeServicesForProjectWorkspaces(db, companyId, projectWorkspaceIds) {
    ...
    return new Map(Array.from(grouped.entries()).map(([workspaceId, workspaceRows]) => [
        workspaceId,
        selectCurrentRuntimeServiceRows(workspaceRows),
    ]));
}
export async function listCurrentRuntimeServicesForExecutionWorkspaces(db, companyId, executionWorkspaceIds) {
    if (executionWorkspaceIds.length === 0)
        return new Map();
    const rows = await db
        .select()
        .from(workspaceRuntimeServices)
        .where(and(eq(workspaceRuntimeServices.companyId, companyId), inArray(workspaceRuntimeServices.executionWorkspaceId, executionWorkspaceIds)))
        .orderBy(desc(workspaceRuntimeServices.updatedAt), desc(workspaceRuntimeServices.createdAt));
    const grouped = new Map();
    for (const row of rows) {
...
//# sourceMappingURL=workspace-runtime-read-model.js.map