...
function sanitizeServiceKeySegment(value, fallback) {
...
function getRuntimeServiceRegistryPath(serviceKey) {
    return path.resolve(getRuntimeServicesDir(), `${serviceKey}.json`);
}
function normalizeRegistryRecord(raw) {
...
async function safeReadRegistryRecord(filePath) {
    try {
...
export function createLocalServiceKey(input) {
...
export async function writeLocalServiceRegistryRecord(record) {
...
export async function removeLocalServiceRegistryRecord(serviceKey) {
    await fs.rm(getRuntimeServiceRegistryPath(serviceKey), { force: true });
}
export async function readLocalServiceRegistryRecord(serviceKey) {
...
export async function findLocalServiceRegistryRecordByRuntimeServiceId(input) {
    const records = await listLocalServiceRegistryRecords(input.profileKind ? { profileKind: input.profileKind } : undefined);
...
async function isLikelyMatchingCommand(record) {
...
export async function findAdoptableLocalService(input) {
    ...
    if (input.command && record.command !== input.command)
        return null;
    if (input.cwd && path.resolve(record.cwd) !== path.resolve(input.cwd))
        return null;
    if (input.envFingerprint && record.envFingerprint !== input.envFingerprint)
        return null;
    if (input.port !== undefined && input.port !== null && record.port !== input.port)
        return null;
    return record;
}
export async function touchLocalServiceRegistryRecord(serviceKey, patch) {
...
export async function terminateLocalService(record, opts) {
...
export async function readLocalServicePortOwner(port) {
...
//# sourceMappingURL=local-service-supervisor.js.map